...
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        take_button = findViewById(R.id.take_button);
        take_button.setOnClickListener(v -> {
            Bitmap bitmapImage = Bitmap.createBitmap(surfaceView.getWidth(), surfaceView.getHeight(), Bitmap.Config.ARGB_8888);
            ...
            this.runOnUiThread(() -> {
                ...
                take_button.setVisibility(View.GONE);
                getSquare.setVisibility(View.VISIBLE);
                surfaceView.onPause();
            });
        });

        getSquare.setOnClickListener(v -> {
            try {
                Thread.sleep(500);
...
//        throw new RuntimeException(e);
            ...
            getSquare.setOnClickListener(a -> {
                onCreateDialog().show();
            });
    ...
    public void onDrawFrame(SampleRender render) {
        ...
        if (camera.getTrackingState() == TrackingState.TRACKING
            ...
            try (Image depthImage = frame.acquireDepthImage16Bits()) {
                ...
                getMillimetersDepth(depthImage, depthImage.getWidth() / 2, depthImage.getHeight() / 2);

                Log.e("CAMERA", getCameraResolution(0).toString());

                ByteBuffer buffer;
                buffer = depthImage.getPlanes()[0].getBuffer();
                Log.e("BUFFER", buffer.toString());
                byte[] bytes = new byte[buffer.capacity()];
                buffer.get(bytes);
                Log.e("BYTES", String.valueOf(bytes[0]));
