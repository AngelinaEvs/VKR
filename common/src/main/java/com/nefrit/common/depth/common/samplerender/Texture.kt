...
class Texture @JvmOverloads constructor(
    ...
    private val textureId: IntArray = intArrayOf(0)
    ...
    init {
        ...
        try {
            GLES30.glBindTexture(target.glesEnum, textureId.get(0))
    ...
    companion object {
        private val TAG: String = Texture::class.java.getSimpleName()
        ...
        fun createFromAsset(
            ...
            val texture: Texture = Texture(render, Target.TEXTURE_2D, wrapMode)
            ...
            try {
                ...
                val buffer: ByteBuffer = ByteBuffer.allocateDirect(bitmap.getByteCount())
        ...
        private fun convertBitmapToConfig(bitmap: Bitmap, config: Bitmap.Config): Bitmap {
            ...
            val result: Bitmap = bitmap.copy(config,  /*isMutable=*/false)
            bitmap.recycle()
            return result
        }
    }
