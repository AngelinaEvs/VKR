...
class PlaneRenderer(render: SampleRender) {
     ...
     */
    init {
        val texture: Texture = Texture.Companion.createFromAsset(
            render, TEXTURE_NAME, WrapMode.REPEAT, ColorFormat.LINEAR
        )
        shader = Shader.Companion.createFromAssets(
            render,
            VERTEX_SHADER_NAME,
        ...
        mesh = Mesh(render, PrimitiveMode.TRIANGLE_STRIP, indexBufferObject, vertexBuffers)
    }
    ...
    fun drawPlanes(
        ...
        Collections.sort(
            sortedPlanes,
            Comparator { a, b -> java.lang.Float.compare(b.distance, a.distance) })
        cameraPose.inverse().toMatrix(viewMatrix, 0)
        for (sortedPlane: SortablePlane in sortedPlanes) {
    ...
    private class SortablePlane internal constructor(val distance: Float, val plane: Plane)
    companion object {
        ...
        private val TEXTURE_NAME = "models/trigrid.png"
        private val BYTES_PER_FLOAT = java.lang.Float.SIZE / 8
        private val BYTES_PER_INT = Integer.SIZE / 8
